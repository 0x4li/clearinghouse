...
<head>
  ...
  <style>
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    header{background:var(--navy);color:white;padding:0 2rem;height:64px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:100;box-shadow:0 2px 12px rgba(0,0,0,.15)}
    ...
    .card-title{font-family:'Space Grotesk',sans-serif;font-size:1rem;font-weight:700;color:var(--navy);margin-bottom:1.25rem;padding-bottom:.75rem;border-bottom:2px solid var(--sky);display:flex;align-items:center;gap:8px}
    ...
    .form-group input{border:1.5px solid var(--gray-2);border-radius:8px;padding:10px 14px;font-size:.95rem;font-family:'Inter',sans-serif;color:var(--text);transition:border-color .2s,box-shadow .2s;outline:none}
    ...
    .score-box input{border:none!important;background:transparent!important;text-align:center;font-size:1.3rem;font-weight:600;color:var(--navy);width:100%;box-shadow:none!important;padding:0!important;outline:none}
    ...
    .submit-btn{width:100%;background:var(--blue);color:white;border:none;border-radius:10px;padding:15px;font-size:1rem;font-weight:600;font-family:'Inter',sans-serif;cursor:pointer;transition:all .2s;display:flex;align-items:center;justify-content:center;gap:8px}
    ...
    #toast{position:fixed;bottom:2rem;left:50%;transform:translateX(-50%) translateY(80px);background:white;border-radius:12px;padding:14px 22px;box-shadow:0 8px 32px rgba(0,0,0,.12);display:flex;align-items:center;gap:12px;font-weight:500;font-size:.9rem;transition:transform .35s cubic-bezier(.34,1.56,.64,1);z-index:999;min-width:300px;max-width:90vw}
  ...
  <div id="form-wrapper">
    ...
    <div class="card">
      ...
      <div class="track-list" id="track-list">
        <div class="track-item" data-track="SecDev" draggable="true"><div class="rank-badge">1</div><div class="track-icon">🔐</div><div class="track-info"><div class="track-name">SecDev</div><div class="track-desc">Security Development</div></div><div class="drag-handle">⠿</div></div>
        <div class="track-item" data-track="NetSec" draggable="true"><div class="rank-badge">2</div><div class="track-icon">🌐</div><div class="track-info"><div class="track-name">NetSec</div><div class="track-desc">Network Security</div></div><div class="drag-handle">⠿</div></div>
        <div class="track-item" data-track="Crypto" draggable="true"><div class="rank-badge">3</div><div class="track-icon">🔑</div><div class="track-info"><div class="track-name">Crypto</div><div class="track-desc">Cryptography</div></div><div class="drag-handle">⠿</div></div>
        <div class="track-item" data-track="GRC" draggable="true"><div class="rank-badge">4</div><div class="track-icon">📋</div><div class="track-info"><div class="track-name">GRC</div><div class="track-desc">Governance, Risk &amp; Compliance</div></div><div class="drag-handle">⠿</div></div>
      </div>
    </div>
...
<script>
  ...
  async function runAllocation() {
    ...
    const rows = data.assignments.map(a => `<tr><td>${a.rank}</td><td><strong>${a.student_id}</strong></td><td>${a.name||'—'}</td><td>${a.s1}</td><td>${a.s2}</td><td>${a.s3}</td><td>${a.s4}</td><td><strong>${a.metric}</strong></td><td>${trackBadge(a.first_choice)}</td><td>${trackBadge(a.assigned_track)}</td><td><span class="badge ${a.got_first_choice?'badge-yes':'badge-no'}">${a.got_first_choice?'✓ Yes':'✗ No'}</span></td></tr>`).join('');
    document.getElementById('results-area').innerHTML = `<p style="color:var(--gray-3);font-size:.82rem;margin-bottom:.75rem">Method ${data.method} · Metric: ${data.metric_label} · ${data.assignments.length} students ranked</p><div class="table-wrap"><table><thead><tr><th>Rank</th><th>Student ID</th><th>Name</th><th>S1</th><th>S2</th><th>S3</th><th>S4</th><th>Metric</th><th>1st Choice</th><th>Assigned</th><th>Got 1st?</th></tr></thead><tbody>${rows}</tbody></table></div>`;
